...
# News
-  *[Apr 2026]* Adding the OFC 2026 ML Challenge competition data for booster and pre-amplifier EDFAs. Check the data at folder "./misc/live_comp_data_new/".
-  *[Jun 2024]* Adding new measurements for multi-span links with three different topologies, with a detailed explanation available in [4]. Check the multi-span link data at folder "./dataset/multispan".
-  *[Jan 2023]* Adding remeasurement data after half a year for all EDFAs @18 dB to the dataset for analyzing the channel gain behaviors versus time. Check the remeasurement data at folder "./misc/remeasurement/". 
...
📦COSMOS-EDFA-Dataset 
 ...
 ┣ 📂dataset  
 ┃ ┣ 📂multispan  
 ┃ ┃ ┣ 📜multispan_topology1.json  
 ┃ ┃ ┣ 📜multispan_topology2.json  
 ┃ ┃ ┣ 📜multispan_topology3.json  
 ┃ ┣ 📂booster  
 ┃ ┃ ┣ 📂15dB  
 ┃ ┃ ┃ ┣ 📂extraLow  
 ...
 ┣ 📂misc  
 ┃ ┣ 📂live_comp_data_new
 ┃ ┃ ┣ 📜edfa_meas_rdm1-co1.bed_booster_2026.02.25.14.08.38.json
 ┃ ┃ ┣ 📜edfa_meas_rdm1-co1.bed_preamp_2026.02.23.10.25.12.json
 ┃ ┃ ┣ ...
 ┃ ┃ ┗ 📜edfa_meas_rdm6-co1.bed_preamp_2026.02.25.12.26.51.json
 ┃ ┣ 📂measurement_config  
...
In the `misc` folder, we put all other related files there. For example, channel loading configuration files are located in the `measurement_config` folder. The `figures` and `ML_features` folders store figures and ML model-readable features in *CSV* format generated by the example code, respectively. The `md_support_materials` folder has all related materials for this document. The `live_comp_data_new` folder stores the OFC 2026 ML Challenge competition data in *JSON* format. These files are timestamped measurements for booster and pre-amplifier EDFAs collected from the deployed ROADMs.
...
[4] Z. Wang, Y.-K. Huang, S. Han, T. Wang, D. Kilper, and T. Chen,"Multi-span optical power spectrum prediction using ML-based EDFA models and cascaded learning," in Proc. IEEE/Optica Optical Fiber Communication Conference (OFC’24), 2024.
